<template>
    <div class="m-paging">
        ...
        <ul class="m-paging__pager">
            ...
            <li class="m-paging__pager--number" :class="{active:currentPage===item.value}" v-for="(item,index ) in pageNumberArr" :key="index" @click="jumpPage(item)">
...
export default {
  ...
  methods: {
    jumpPage: function(item) {
      ...
      if (_type === "number") {
        ...
        if (_value < this.currentPage) {
          this.currentPage = this.currentPage - Math.ceil(this.pageCount / 2) - 1;
        } else {
          this.currentPage = this.currentPage + Math.ceil(this.pageCount / 2) + 1;
        }
        this.pageNumberArr = this.computePageNumberArr();
    ...
    computePageNumberArr: function() {
      ...
      } else if (_currentPage > LASTPAGE - PAGECOUNT) {
        arr.push({
          index: 1,
          value: 1,
          type: "number"
